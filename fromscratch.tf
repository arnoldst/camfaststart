#####################################################################
##
##      Created 1/10/19 by admin.
##
#####################################################################

## REFERENCE {"SDDC-DPG-Mgmt":{"type": "vsphere_reference_network"}}
...
variable "SDDC-DPG-Mgmt_network_name" {
  type = "string"
  description = "Generated"
}
...
data "vsphere_network" "SDDC-DPG-Mgmt" {
  name          = "${var.SDDC-DPG-Mgmt_network_name}"
  datacenter_id = "${data.vsphere_datacenter.virtual_machine_datacenter.id}"
}
...
resource "vsphere_virtual_machine" "virtual_machine" {
  ...
  network_interface {
    network_id = "${data.vsphere_network.SDDC-DPG-Mgmt.id}"
  }
  clone {
